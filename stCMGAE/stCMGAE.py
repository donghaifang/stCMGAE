...
class stCMGAE:
    ...
    def train(self):
        ...
        self.model = stCMGAE_model(features_dims, bn=self.__bn,
                                  att_dropout_rate=self.__att_dropout_rate,
                                  use_token=self.__use_token, alpha=self.__alpha,
                                  feat_mask_rate=self.__feat_mask_rate,momentum=self.__momentum).to(self.__device)
        ...
        for epoch in epoch_iter:
            ...
            feat_loss, dig_loss = self.model(data)
            loss = (1 - self.__lam) * feat_loss + self.__lam * dig_loss
            loss.backward()
    ...
    def clustering(self, method="kmeans"):
            if method == "mclust":
